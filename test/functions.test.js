const { checkNumber,checkNumber2, findMax, getStatus,getStatus2, switchExample, loopExample, strictEquality, specialPhrases1,specialPhrases2, specialPhrases3,isPrime,isPrime2, looseEquality } = require('../src/functions');

test('checkNumber should return "even" for even numbers', () => {
  expect(checkNumber(4)).toBe('even');
});

test('checkNumber should return "odd" for odd numbers', () => {
    expect(checkNumber(3)).toBe('odd');
});

test('checkNumber2 should return "even" for even numbers', () => {
    expect(checkNumber2(4)).toBe('even');    
});

test('checkNumber2 should return "odd" for odd numbers', () => {
...
test('getStatus should return "active" for status code 1, "pending" for 2, and "unknown" otherwise', () => {
  expect(getStatus(1)).toBe('active');
  expect(getStatus(2)).toBe('pending');
  expect(getStatus(5)).toBe('unknown');
});

test('getStatus2 should return "active" for status code 1, "pending" for 2, and "unknown" otherwise', () => {
...
test('switchExample should categorize number ranges correctly, handle edge values', () => {
  ...
  expect(switchExample(20)).toBe('high');
    expect(switchExample(30)).toBe('unknown');
});
...
test('isPrime2 should correctly identify prime numbers', () => {
